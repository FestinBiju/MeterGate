...
The buyer signs in to the web application using the normal HttpOnly session cookie and creates a short-lived Agent Session. MeterGate returns its `mcp_…` credential once, stores only a SHA-256 hash, and records account, scope, expiry, last-use, and revocation metadata. The MCP process receives that separate credential through its environment. It never receives the browser cookie or CSRF secret.
