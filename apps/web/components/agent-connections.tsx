...
export function AgentConnections() {
  ...
  const [pendingAction, setPendingAction] = useState<
    { kind: "create" } | { kind: "revoke"; sessionId: string } | null
  >(null);
  ...
  return (
    <section className="mt-10 rounded-3xl border border-cyan-300/15 bg-cyan-300/[0.025] p-5 sm:p-6" aria-labelledby="agent-connections-heading">
      ...
      <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
        Create a short-lived MCP bridge for this buyer account. It can orchestrate only the selected scopes; passkey approval, Razorpay Checkout, refunds, and operator controls remain unavailable.
      </p>
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="agent-session-confirmation" className="mt-4 rounded-2xl border border-amber-300/25 bg-amber-300/[0.06] p-4">
          ...
          <p className="mt-2 text-xs leading-5 text-slate-300">
            {pendingAction.kind === "create"
              ? "Create a 15-minute buyer-agent credential with the displayed scopes? It cannot approve purchases, operate Razorpay, issue refunds, or access operator APIs."
              : `Revoke agent session ${pendingAction.sessionId} immediately? Every later scoped tool call using it will fail.`}
          </p>
          <div className="mt-3 flex gap-2">
            {pendingAction.kind === "create" ? (
              <HumanPresenceGate
                ...
                disabled={busy}
                resourceBinding={{ scopes: selectedScopes, expires_in_seconds: 900 }}
            ...
            <button type="button" disabled={busy} onClick={() => setPendingAction(null)} className="rounded-lg border border-white/10 px-3 py-2 text-xs text-slate-300 disabled:opacity-50">
              Cancel
            </button>
          </div>
        </div>
      ...
      <div className="mt-6 space-y-3">
        {sessions.length === 0 ? (
          ...
          sessions.map((session) => (
            <article key={session.id} className="rounded-xl border border-white/[0.07] bg-black/15 p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                {session.state === "active" ? (
                  <button type="button" disabled={busy} onClick={() => setPendingAction({ kind: "revoke", sessionId: session.id })} className="rounded-lg border border-rose-300/20 px-3 py-2 text-xs text-rose-100 disabled:opacity-50">
